...
export class Img2TextModel extends BaseMultimodalModel {
  ...
  init = async (
    ...
    const imageEncoderOutputName =
      this.metadata.outputNames?.get("image-encoder");
    ...
    this.imageEncoder = new Encoder(
      imageEncoderSession,
      imageEncoderOutputName,
      GeneratorType.Img2Seq
    );
    if (this.metadata.modelPaths.has("text-encoder")) {
      ...
      const textEncoderOutputName =
        this.metadata.outputNames?.get("text-encoder");
      if (!textEncoderOutputName) {
        throw new Error("output names do not have the 'encoder' path");
      }
      const textEncoderSession = await createSession(textEncoderPath, proxy);
      this.textEncoder = new Encoder(
        textEncoderSession,
        textEncoderOutputName,
  ...
  process = async (
    ...
    const imageTensor = await prepareImagesTensor(
      imageInputs,
      this.preprocessor
    );
    if (
