...
export class TextFeatureExtractionModel extends BaseTextModel {
  ...
  process = async (inputs: string | string[]): Promise<TextFeatureExtractionResult> => {
  ...
  private generate_output = async (
    ...
    for (let idx = 0; idx < lastHiddenState.dims[0]; idx++) {
      ...
      for (let i = 0; i < lastHiddenState.dims[1]; i++) {
        const attnValue = attentionMask.data[idx * lastHiddenState.dims[1] + i] as number;
        if (attnValue) {
  ...
  private run_dense = async (pooledResult: number[][]): Promise<number[][]> => {
    ...
    const tensor = new ort.Tensor("float32", new Float32Array(flatten), [pooledResult.length, pooledResult[0].length]);
