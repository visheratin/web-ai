...
export const encodeData = async (
  ...
  if (textEncoder) {
    ...
    const textOutput = await textEncoder.process(
      textInput,
      textAttention,
      imageOutput
    );
...
export async function* generate(
  ...
  let decoderAttention = new ort.Tensor(
    "int64",
    new BigInt64Array(encoderOutput.dims[0]).fill(1n),
    [encoderOutput.dims[0], 1]
  );
  if (initDecoderInput) {
  ...
  while (true) {
    const decoderOutput = await decoder.process(
      encoderOutput,
      decoderInput,
      decoderAttention,
      inputAttentionMask
    ...
    for (let i = 0; i < decoderInput.dims[0]; i++) {
      ...
      if (
        newTokenIDs[i] === options.eosTokenID ||
    ...
    decoderInput = new ort.Tensor(
      "int64",
      new BigInt64Array(newDecoderTokens),
      [decoderInput.dims[0], decoderInput.dims[1] + 1]
    );
    decoderAttention = new ort.Tensor(
      "int64",
      new BigInt64Array(newDecoderAttention),
      [decoderAttention.dims[0], decoderAttention.dims[1] + 1]
    );
    len += 1;
