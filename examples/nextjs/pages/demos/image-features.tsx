...
export default function ImageFeatures() {
  ...
  const selectFileImage = (first: boolean) => {
    if (first) {
      if (
        fileSelect1Ref.current &&
        fileSelect1Ref.current.files &&
        fileSelect1Ref.current.files[0]
      ) {
      ...
      if (
        fileSelect2Ref.current &&
        fileSelect2Ref.current.files &&
        fileSelect2Ref.current.files[0]
      ) {
        const reader = new FileReader();
        reader.onload = async () => {
          const data = reader.result as ArrayBuffer;
  ...
  const process = async () => {
    if (
      !imageData1 ||
      imageData1.byteLength == 0 ||
      !imageData2 ||
      imageData2.byteLength == 0 ||
      !model
    ) {
      return;
    }
              ...
              <div className="row mb-2">
                ...
                <div className="col-sm-6">
                  <p className="text-center">
                    Simimarity score of the images is {result.value}.
                  </p>
                </div>
              </div>
            </div>
